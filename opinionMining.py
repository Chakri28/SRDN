...
class opinionMining(nn.Module):
    def __init__(self, args, config, label_alphabet):
        ...
        # target syn
        self.targetSyn_r = nn.Parameter(torch.Tensor(
            self.target_hidden_dim, self.bert_encoder_dim))
        self.targetSyn_s = nn.Parameter(torch.Tensor(
            self.target_hidden_dim, self.bert_encoder_dim))
        # relation syn
        self.relationSyn_u = nn.Parameter(torch.Tensor(
            self.relation_hidden_dim, self.bert_encoder_dim))
        self.relationSyn_s = nn.Parameter(torch.Tensor(
            self.relation_hidden_dim, self.bert_encoder_dim))
        ...
        # crf
        self.targetHidden2Tag = nn.Parameter(torch.Tensor(
            self.label_size + 2, self.target_hidden_dim))
    ...
    def neg_log_likelihood_loss(self, all_input_ids, all_segment_ids, all_labels, all_relations, all_input_mask):
        ...
        maskTemp1 = all_input_mask.view(
            batch_size, 1, seq_len).repeat(1, seq_len, 1)
        maskTemp2 = all_input_mask.view(
            batch_size, seq_len, 1).repeat(1, 1, seq_len)
        ...
        target_loss = self.crf.neg_log_likelihood_loss(
            targetPredictScore, all_input_mask.byte(), all_labels)
        scores, tag_seq = self.crf._viterbi_decode(
        ...
        relationScoreLoss = torch.cat(
            [1 - relationScoreLoss, relationScoreLoss], 1)
        relation_loss = relation_loss_function(
            relationScoreLoss, relationlabelLoss)
    ...
    def mainStructure(self, maskMatrix, all_input_ids, all_segment_ids, steps, all_input_mask):
        ...
        for i in range(steps):
            ...
            target_div = target_div.unsqueeze(
                2).repeat(1, 1, self.bert_encoder_dim)
            target_r = torch.div(target_weighted, target_div)
            target_hidden = F.linear(
                sequence_output, self.targetSyn_s, None) + F.linear(target_r, self.targetSyn_r, None)
            ...
            relation_div = relation_div.unsqueeze(
                2).repeat(1, 1, self.bert_encoder_dim)
            relation_a = torch.div(relation_weighted, relation_div)
            relation_hidden = F.linear(
                sequence_output, self.relationSyn_s, None)+F.linear(relation_a, self.relationSyn_u, None)
            relation_hidden = F.tanh(relation_hidden)

            # crf
            # self.targetHidden2Tag(target_hidden)
            targetPredictInput = F.linear(
                target_hidden, self.targetHidden2Tag, self.targetHidden2Tag_b)

            # Relation Attention
            relationScore = self.relationAttention(relation_hidden)

            # update T_tensor
            tag_score, tag_seq = self.crf._viterbi_decode(
                targetPredictInput, all_input_mask.byte())
            threads = []
            temp_T_tensor = torch.zeros(batch_size, seq_len, seq_len)
            if self.gpu:
                temp_T_tensor = temp_T_tensor.cuda()
            for i in range(batch_size):
                t = threading.Thread(target=self.makeEntity, args=(
                    i, tag_seq[i, :], temp_T_tensor, seq_len))
                threads.append(t)
            for i in range(batch_size):
                threads[i].start()
            for i in range(batch_size):
                threads[i].join()
            tag_score_final = tag_score.unsqueeze(2).repeat(
                1, 1, seq_len)+tag_score.unsqueeze(1).repeat(1, seq_len, 1)
    ...
    def makeEntity(self, idx, tag_seq, temp_T_tensor, seq_len):
        ...
        for idy in range(seq_len):
            if tag_seq[idy] in [0, 1, 2, 4]:
                if Abegin != -1:
                    temp_T_tensor[idx, Abegin:Aend, Abegin:Aend] = torch.ones(
                        Aend - Abegin, Aend - Abegin)
                    Abegin = -1
                    Aend = -1
                if Obegin != -1:
                    temp_T_tensor[idx, Obegin:Oend, Obegin:Oend] = torch.ones(
                        Oend - Obegin, Oend - Obegin)
                    Obegin = -1
                    Oend = -1
            if tag_seq[idy] == 2:
                Abegin = idy
