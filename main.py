...
def relationPredictCheck(relationPredict, batch_relation):
    relationCheck = torch.ones(relationPredict.size(
        0), relationPredict.size(1), relationPredict.size(2)) * 0.1
...
def recover_label(targetPredict, all_labels, all_input_mask):
    ...
    for idx in range(batch_size):
        pred = [pred_tag[idx][idy] -
                1 for idy in range(seq_len) if mask[idx][idy] != 0]
        gold = [gold_tag[idx][idy] -
                1 for idy in range(seq_len) if mask[idx][idy] != 0]
    ...
    for idx in range(len(instance_text)):
        ...
        for o in opinionList:
            for t in targetList:
                # *(t[1]-t[0]))
                score1 = np.sum(
                    R_tensor[idx][o[0]:o[1], t[0]:t[1]]) / (o[1] - o[0])
                # *(t[1]-t[0]))
                score2 = np.sum(
                    R_tensor[idx][t[0]:t[1], o[0]:o[1]]) / (t[1] - t[0])
                if (score1 + score2) / 2 > thred:
                    if [o[0] - 1, o[1] - 1, t[0] - 1, t[1] - 1] not in relationResult:
                        relationResult.append(
                            [o[0] - 1, o[1] - 1, t[0] - 1, t[1] - 1])
...
def evaluate(eval_dataloader, test_set, model, output_file_path, args):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        targetPredict, relationPredict = model(
    ...
    # evaluate
    TP, TR, TF, OP, OR, OF = get_ner_fmeasure(
        gold_results, pred_results, args.tagScheme)
    pred_relations = make_relation(
        relation_result, pred_results, args.inference_threds)
    ...
    for k in range(len(test_set)):
        ...
        for j in range(len(gold)):
            output_file.write(
                words[j + 1] + "\t" + gold[j] + "\t" + labelDic[pred[j + 1]] + "\n")
...
def read_data(train_features, type, batchsize):
    ...
    all_input_ids = torch.tensor(
        [f.token_ids for f in train_features], dtype=torch.long)
    all_input_mask = torch.tensor(
        [f.token_mask for f in train_features], dtype=torch.long)
    all_segment_ids = torch.tensor(
        [f.segmentId for f in train_features], dtype=torch.long)
    all_relations = torch.tensor(
        [f.relations for f in train_features], dtype=torch.long)
    all_labels = torch.tensor(
        [f.label_ids for f in train_features], dtype=torch.long)
    all_labels[:, :1] = torch.ones(all_labels.size(0), 1).long()
    if type == "train":
        train_data = TensorDataset(
            all_input_ids, all_input_mask, all_segment_ids, all_relations, all_labels)
    ...
    train_dataloader = DataLoader(
        train_data, sampler=train_sampler, batch_size=batchsize)
...
def main(args):
    ...
    if args.mode == "test":
        ...
        model = bert_load_state_dict(model, torch.load(
            args.bert_checkpoint_dir, map_location='cpu'))
        ...
        for idx in range(args.iteration):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                all_relations = all_relations[:,
                                              :max_seq_len, :max_seq_len].contiguous()
                all_labels = all_labels[:, :max_seq_len].contiguous()
                ...
                if step % 20 == 0:
                    ...
                    print("     Instance: %s; Time: %.2fs; loss: %.4f; target_acc: %s/%s=%.4f; relation_acc: %s/%s=%.4f" % (
                        ...
                        (right_target_token + 0.) /
                        whole_target_token, right_relation_token, whole_relation_token,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_model', type=str,
                        default="./model/2014Lap/modelFinal.model")
    parser.add_argument('--test_eval_dir', type=str,
                        default="./test_eval/2014Lap")
